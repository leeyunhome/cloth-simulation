...
public:
    ...
    // Project any penetrating particle back onto the sphere surface.
    void collideSphere(const Vec3& center, float radius) {
        ...
        for (std::size_t i = 0; i < pos_.size(); ++i) {
            ...
            if (dist < r && dist > 1e-8f) pos_[i] = center + d * (r / dist);
        }
    }
